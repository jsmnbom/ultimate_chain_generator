"""BREP measurement primitives for the viewer's Distance / Properties tools.

Vendored verbatim (bar formatting) from ``ocp_vscode.measure`` — copyright 2025
Bernhard Walter, Apache-2.0 (github.com/bernhard-42/vscode-ocp-cad-viewer). We
vendor rather than ``pip install ocp_vscode`` because that package pulls a heavy,
browser-unusable dependency tree (flask, websockets, requests, ipykernel, …); the
measurement code itself only needs ``OCP`` and ``ocp_tessellate.ocp_utils``, both
already present in the Pyodide worker. ``runtime.py`` calls ``get_distance`` /
``get_properties`` from here; keep this in sync if the pinned versions change.
"""
...
def get_geom_type(shape):
  if is_topods_vertex(shape) or is_vector(shape):
    ...
    return get_curve(shape).GetType().name.split("_")[-1]  # type: ignore[union-attr]
  elif is_topods_face(shape):
    return get_surface(shape).GetType().name.split("_")[-1]  # type: ignore[union-attr]
  elif (
    is_topods_solid(shape) or is_topods_compound(shape) or is_topods_compsolid(shape)
  ):
    return "Other"
  else:
    print(shape, "unknown")
...
def get_properties(shape):
  ...
  if shape_type == "Vertex":
    ...
    if geom_type == "Plane":
      ...
      geom_section["axe loc"] = get_point(revolution.AxeOfRevolution().Location())  # type: ignore[union-attr]
      _axe_dir = revolution.AxeOfRevolution().Direction()  # type: ignore[union-attr]
      geom_section["axe dir"] = (_axe_dir.X(), _axe_dir.Y(), _axe_dir.Z())
...
def calc_angle(shape1, shape2):
  ...
  response: dict[str, Any] = {"angle": angle}
