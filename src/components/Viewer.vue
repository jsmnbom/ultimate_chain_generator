...
import type { Shapes, ChangeNotification } from "three-cad-viewer";
...
function handleNotify(changed: ChangeNotification) {
  ...
  if (changed.position) keptPosition = changed.position.new as number[];
  if (changed.quaternion) keptQuaternion = changed.quaternion.new as number[];
  if (changed.target) keptTarget = changed.target.new as number[];
  if (changed.zoom) keptZoom = changed.zoom.new as number;
  ...
  if (relevant) props.measure.send(changes);
}
...
function chromeHeight() {
  ...
  if (!cad || !view) return 2; // elements not built yet; first render corrects it
...
function keepViewOptions(shapes: Shapes) {
  const bb = shapes.bb;
  if (!keptPosition || !keptTarget || !bb) return null;
...
function onResize() {
  ...
  if (!viewer || !hasRendered || !host.value) return;
  const { width, height } = dims();
...
onMounted(() => {
  createViewer();
  if (props.shapes) renderShapes(props.shapes);
