...
async function initialize(): Promise<void> {
  ...
  pyodide.registerJsModule("host_bridge", {
    send_data_to_js(data: string, msgType: string): void {
      if (msgType === "DATA" && currentBuildPort) post(currentBuildPort, { type: "shapes", data });
    },
  });
...
function schedule() {
  ...
  if (meas) {
    ...
    doMeasure(meas.changes)
      .then((response) => {
        if (response) post(meas.port, { type: "measure-response", response });
      })
...
function cleanupPort(port: MessagePort) {
  ...
  for (let i = exportQueue.length - 1; i >= 0; i--) {
    if (exportQueue[i].port === port) exportQueue.splice(i, 1);
  }
  for (let i = measureQueue.length - 1; i >= 0; i--) {
    if (measureQueue[i].port === port) measureQueue.splice(i, 1);
  }
}
