...
if (!reqMatch) throw new Error("Could not find REQUIREMENTS = [...] in install.py");
const requirements = [...reqMatch[1].matchAll(/"([^"]+)"/g)].map((m) => m[1]);

const mockMatch = /# === MOCKS START[^\n]*\n([\s\S]*?)\n# === MOCKS END ===/.exec(py);
if (!mockMatch) throw new Error("Could not find MOCKS START/END markers in install.py");
...
for (const p of packages) {
  if (!String(p.file_name).startsWith("http")) continue;
